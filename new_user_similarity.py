...
def make_new_user_df(user_interest):
...
def find_similar_index(user_interest, item_interest, outdoor_interest):
    df_features = make_new_user_df(user_interest) # 기존 유저의 관심 아이템 + 관심 아웃도어의 전처리 작업
    ...
    if not exact_match_indices:
        ...
        similarities = cosine_similarity([new_data], combined_interest_matrix) # 새로운 데이터와의 코사인 유사도 계산
        most_similar_index = np.argmax(similarities) # 가장 유사한 인덱스 찾기
