...
def find_similar_user_interest(item, case2_dict, user_interest, item_interest, outdoor_interest):
    ...
    # 만약 기존 유저와 관심 항목이 일치하는게 없다면 if, 없으면 else
    if not exact_match_indices:
        ...
        # match_idx 중 case2_user_place에서 가장 행동이 많은 유저 선택
        if match_idx:
            user_actions = case2_dict[item][5].loc[match_idx].apply(lambda row : (row != 0).sum(), axis=1)
            user_id = int(user_interest[user_interest.index == user_actions.idxmax()].iloc[:,0])
